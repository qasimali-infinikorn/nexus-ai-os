...
export function GoogleCalendarCard({ configured, connected, accountEmail, calendarQuery }: Props) {
  const [pending, startTransition] = useTransition();
  const [syncState, setSyncState] = useState<{ error?: string; success?: string } | undefined>();

  const banner =
    calendarQuery === "not_configured"
      ? "Set GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET to enable Calendar connect."
  ...
  return (
    <article className="card card-pad stack-md">
      <div className="row" style={{ gap: 12 }}>
        <Avatar initials="GC" index={4} square />
        <div className="stack" style={{ minWidth: 0 }}>
          <span className="card-title">Google Calendar</span>
          <span className="card-sub">Meetings · personal OAuth (not login)</span>
        </div>
      </div>
      <p className="dim" style={{ fontSize: "var(--fs-body)", lineHeight: 1.55 }}>
        Sync the next 14 days of primary calendar events into Meeting Assistant. Tokens are encrypted
        per user and org — this does not change how you sign in to Nexus.
      </p>
      ...
      <div className="row-between" style={{ marginTop: "auto", flexWrap: "wrap", gap: 8 }}>
        ...
        <div className="row" style={{ gap: 8 }}>
          {connected ? (
              ...
              <button
                ...
                onClick={() => {
                  startTransition(async () => {
                    setSyncState(await syncGoogleCalendarAction());
                  });
                }}
              >
            ...
            <Link href="/api/integrations/google-calendar/start" className="btn-primary btn-sm">
              Connect
            </Link>
          ) : (
