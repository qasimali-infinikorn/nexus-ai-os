...
const BUILTIN = agents.filter((a) => a.agentType).map((a) => ({
  key: a.agentType!,
  name: a.name
}));

export type WorkspaceAgentOption = { key: string; name: string };

export function AIWorkspaceClient({
  customAgents = []
}: {
  customAgents?: WorkspaceAgentOption[];
}) {
  const searchParams = useSearchParams();
  const { provider, model, setProvider, setModel } = useProviderPreference();

  const runnable = [...BUILTIN, ...customAgents];
  const initialAgent = searchParams.get("agent")?.trim();
  const [agentType, setAgentType] = useState(() => {
    if (initialAgent && (initialAgent === "coordinator" || runnable.some((a) => a.key === initialAgent))) {
      return initialAgent;
    }
    return "coordinator";
  });
  ...
  const activeAgentName =
    agentType === "coordinator"
      ? "Coordinator"
      : runnable.find((a) => a.key === agentType)?.name ?? "Agent";
        ...
        <aside className="rail" aria-label="Prompts and history">
          ...
          <section className="rail-section">
            ...
            <p className="muted" style={{ fontSize: "0.76rem" }}>
              Demo history — live runs appear on the Agents page.
            </p>
          </section>
        </aside>
      </div>
