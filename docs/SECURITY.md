...
limiting). Google and Microsoft Calendar OAuth for Meetings are live
(per-user connections). Notification email (Resend) and Slack incoming
webhooks honor the Settings → Notifications matrix when configured.
